...
TEST_CASE("MatchingEngine scenarios", "[matching_engine]") {
    ...
    SECTION("Scenario 1: No match") {
        engine.submit_order(std::make_unique<Order>(1, to_symbol("AAPL"), Side::Buy, OrderType::Limit, 100, 10, 1000));
        engine.submit_order(std::make_unique<Order>(2, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 101, 10, 1001));
        CHECK(fills.empty());
    }

    SECTION("Scenario 2: Exact match") {
        engine.submit_order(std::make_unique<Order>(1, to_symbol("AAPL"), Side::Buy, OrderType::Limit, 100, 10, 1000));
        engine.submit_order(std::make_unique<Order>(2, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 100, 10, 1001));
    ...
    SECTION("Scenario 3: Partial fill") {
        engine.submit_order(std::make_unique<Order>(1, to_symbol("AAPL"), Side::Buy, OrderType::Limit, 100, 10, 1000));
        engine.submit_order(std::make_unique<Order>(2, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 100, 6, 1001));
        REQUIRE(fills.size() == 1);
        CHECK(fills[0].fill_qty == 6);
        
        // Verify residual of order 1 is 4 by matching it with a new sell order
        engine.submit_order(std::make_unique<Order>(3, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 100, 4, 1002));
    ...
    SECTION("Scenario 4: Multi-level sweep") {
        engine.submit_order(std::make_unique<Order>(1, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 101, 10, 1000));
        engine.submit_order(std::make_unique<Order>(2, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 102, 10, 1001));
        engine.submit_order(std::make_unique<Order>(3, to_symbol("AAPL"), Side::Buy, OrderType::Market, 0, 15, 1002));
    ...
    SECTION("Scenario 6: Cancel non-existent") {
        ...
        engine.submit_order(std::make_unique<Order>(10, to_symbol("AAPL"), Side::Buy, OrderType::Limit, 100, 10, 1000));
        engine.submit_order(std::make_unique<Order>(11, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 100, 10, 1001));
        REQUIRE(fills.size() == 1);
    ...
    SECTION("Scenario 7: Replace and Time Priority") {
        ...
        engine.submit_order(std::make_unique<Order>(2, to_symbol("AAPL"), Side::Buy, OrderType::Limit, 100, 10, 1001));
        ...
        engine.submit_order(std::make_unique<Order>(4, to_symbol("AAPL"), Side::Sell, OrderType::Limit, 100, 10, 1006));
